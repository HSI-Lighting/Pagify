...
impl<'a> PdfDocument<'a> {
    ...
    ///
    /// **Pagify patch — the only change made to this crate.** Upstream keeps this
    /// `pub(crate)`, which puts three PDFium functions out of reach even though
    /// all three are declared on the public `PdfiumLibraryBindings` trait:
    ///
    /// - `FPDF_SaveWithVersion` with `FPDF_INCREMENTAL` — `save_to_writer` below
    ///   hardcodes its flags to `0`, which is a full rewrite, and a rewrite
    ///   relocates every object and destroys any signature's byte range;
    /// - `FPDFPage_Delete` — no page deletion is exposed at all;
    /// - `FPDF_MovePages` — nor any reordering.
    ///
    /// Making the handle public unblocks all three at once, where adding a save
    /// variant would have unblocked one. See `rust/vendor/README.md`.
    ///
    /// # Safety
    /// The handle is owned by this `PdfDocument` and is invalidated when the
    /// document is dropped. Callers must not close it or let it outlive them.
    #[inline]
